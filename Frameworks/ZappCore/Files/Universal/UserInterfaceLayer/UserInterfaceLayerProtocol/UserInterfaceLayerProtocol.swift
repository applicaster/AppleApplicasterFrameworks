...
/// The UserInterfaceLayerProtocol protocol defines methods that allow you to implement UI Layer plugin inside Zapp infrustrature .
...
public protocol UserInterfaceLayerProtocol {
    ...
    ///   - applicationData: Dictionary of general configuration of the application that  may  needed to initialization or usage  UI Layer  plugin
    ...
    /// - Note: Plugin in this method must called logic that needs to be done before UI Layer plugin will be presented. Like load `River.json` that has information about screens to present.
    ///  Do not use in this func any loaders that may be called later.
    /// - Attention: Application will wait completion of this func to present application. Completion must be called as soon as  possible
    /// - Parameter completion: Completion handler that notify app level that component  ready to  be presented or fail
    /// - Parameter viewController: ViewController instance of root view  controller of the UI Layer plugin, in case plugin failed to present must call nil
    /// - Parameter error: Error handler in case plugin can not be presented for some reason, Error must  be provided  in case fail, otherwise nil
