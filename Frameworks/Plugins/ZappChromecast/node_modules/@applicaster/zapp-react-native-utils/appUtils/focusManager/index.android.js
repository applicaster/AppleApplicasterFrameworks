...
export const focusManager = (function() {
  ...
  function setInitialFocus() {
    ...
    if (preferredFocus && preferredFocus.length > 0) {
      const focusableItem = preferredFocus[0].component;
      if (focusableItem) {
  ...
  function unregisterGroup(id) {
    const { children } = focusableTree.findInTree(id) || {};
    if (children) {
      children.forEach(({ id: childId }) =>
        focusableTree.findAndRemoveNode(childId)
      );
    }
  ...
  async function moveFocus(direction) {
    if (currentFocusNode) {
      ...
      const nextFocusable = await findFocusableNode(
        direction,
        currentFocusNode
      );
      if (
        ...
        const nextRrect = await nextFocusable.component.getRect();
