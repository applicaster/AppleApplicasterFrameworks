...
export function ComponentsMapComponent(props: Props) {
  ...
  const [
    currentOffsetForComponent,
    setCurrentOffsetForComponent,
  ] = React.useState({});
  ...
  React.useEffect(() => {
    // Saving focusedIndex into the ref to avoid issue with stale closure
  ...
  const saveCurrentOffset = (offset, id) => {
    const newOffsetObj = { ...currentOffsetForComponent };
    newOffsetObj[id] = offset;
    setCurrentOffsetForComponent(newOffsetObj);
  };
  ...
  const onRiverElementFocus = (element, { index }) => {
    ...
    flatListRef?.current?.scrollToIndex({
      animated: true,
      index: index,
      viewOffset: -currentOffsetForComponent?.[element?.props?.id] || 0,
    });
  };

  const scrollByOffset = (offset, saveOffset, id) => {
    if (saveOffset) {
      saveCurrentOffset(offset, id);
    }

    if (offset >= 0 && focusedIndexRef.current >= 0) {
      flatListRef?.current?.scrollToIndex({
        animated: true,
